...
function getEntryMap(fileList) {
  ...
  fileList.filter((item) => {
    ...
    const name = item.fileName.slice(entryPrefix.length).split('.').slice(0, -1).join('');
...
function getAllFile(p, list, level) {
  let files = [];
  if (fs.existsSync(p)) { // 是否存在此路径  && file.endsWith('.js')
    files = fs.readdirSync(p);
    files.forEach((file) => {
      const currPath = `${p}/${file}`;
      if (fs.statSync(currPath).isDirectory()) { // recurse 查看文件是否是文件夹
